...
import { createUserGroup, updateUserGroup, type UserGroup } from "@/lib/user-groups-api";

export interface UserGroupFormDialogProps {
  open: boolean;
  userGroup: UserGroup | null;
  onClose: () => void;
  onSuccess: (savedUserGroup: UserGroup, mode: "add" | "edit") => void;
...
export function UserGroupFormDialog({
  ...
  const mode: "add" | "edit" = userGroup ? "edit" : "add";
  ...
  const handleSubmit = async () => {
    ...
    const result =
      mode === "edit" && userGroup
        ? await updateUserGroup(userGroup.id, { name: trimmed })
        : await createUserGroup({ name: trimmed });
